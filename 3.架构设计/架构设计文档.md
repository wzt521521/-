...
```mermaid
flowchart TD
    ROOT("<b>职业能力大数据服务平台</b><br/>开发视角 · 五层架构总览")
    ...
    L3 --> M31("3.1 爬虫采集模块<br/>Scrapy / Flask")
    L3 --> M32("3.2 消息队列模块<br/>Kafka")
    L3 --> M33("3.3 ETL数据清洗模块<br/>Spark")
    L3 --> M34("3.4 离线分析计算模块<br/>Spark SQL · Hive · MapReduce")
    ...
    L4 --> M43("4.3 分布式文件系统<br/>HDFS")
    L4 --> M44("4.4 数据仓库<br/>Hive")
    L4 --> M45("4.5 实时查询库<br/>HBase")
    ...
    L5 --> M51("5.1 容器化部署<br/>Docker · Compose")
    L5 --> M52("5.2 反向代理与负载均衡<br/>Nginx")
    L5 --> M53("5.3 服务注册与协调<br/>Zookeeper · Kraft")
    L5 --> M54("5.4 监控与日志<br/>ELK · cAdvisor")
```

> **读图说明：** 自上而下 5 层树形结构，根节点逐层分支。层节点汇聚上一层全部模块后，再分支到本层各模块。完整表达了"上层依赖下层、模块逐级收敛"的依赖关系。
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向下 | 后端 `2.1 用户与权限服务` | `POST /api/auth/login` — 账号密码登录 | `{username, password}` → `{token, userInfo}` |
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向下 | 后端 `2.2 数据分析服务` | `GET /api/dashboard/overview` — 概览数据 | 全量聚合指标 |
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向下 | 后端 `2.2 数据分析服务` | `GET /api/positions?keyword=&city=&salary=&edu=&exp=&page=&size=` | 分页岗位列表 |
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向下 | 后端 `2.3 报告生成服务` | `POST /api/reports/generate` — 生成报告 | `{template, timeRange, dimensions[]}` → `{reportId}` |
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向下 | 后端 `2.1 用户与权限服务` | `GET/POST/PUT/DELETE /api/admin/users` — 用户CRUD | 用户对象 |
...
| 向下 | 后端 `2.5 采集管理服务` | `GET /api/admin/collect/tasks` — 采集任务状态 | 任务列表 |
...
> 技术栈：Spring Boot 2.7+ + Spring MVC + Spring Data JPA + Spring Security + Redis + Kafka
...
| 横向 | 所有后端服务 | 提供 JWT 公钥 / Token 校验端点 | 服务间调用 |
...
| 横向 | 报告服务 `2.3`、推荐引擎 `2.4` | Feign / 内部调用 | Java DTO |
...
| 横向 | 数据分析服务 `2.2` | 调用统计接口获取数据 | Java DTO |
...
| 横向 | 数据分析服务 `2.2` | 获取岗位画像数据 | Java DTO |
...
| 横向 | 数据处理层 `3.1 爬虫采集` | 通过数据库配置 + Kafka 指令下发 | |
| 向下 | 存储层 `4.1 MySQL` | 存储采集源配置 + 任务日志 | |

**具体功能：**

- 采集源配置：URL 模板、翻页规则（CSS Selector/XPath）、采集频率（Cron 表达式）
- 任务管理：创建、启动、暂停、删除采集任务
- 调度引擎：集成 Quartz / Spring Task，按 Cron 触发
- 监控统计：日采集量、成功率、失败率、异常日志
- 失败处理：失败重试策略（最多 3 次，指数退避 1min/5min/15min），超阈值钉钉/邮件告警
...
| 横向 | 数据分析服务 `2.2`、报告服务 `2.3` | 请求转发 | HTTP |
...
> 技术栈：Python Scrapy/Flask + Kafka + Spark 3.x + Hive + MapReduce
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 向上 | `2.5 采集管理服务` | 读取 MySQL 采集配置表 + 接收 Kafka 指令 | 配置 JSON / 指令消息 |
| 横向 | `3.2 消息队列` | 将采集结果推送到 Kafka Topic: `raw-job-data` | JSON 格式岗位数据 |
| 向下 | 招聘网站 | HTTP 请求 + HTML 解析 | |

**具体功能：**

- Scrapy 爬虫引擎（每个招聘网站一个 Spider）
- 定时采集：从 MySQL 读取 Cron 配置，APScheduler 调度
- 增量采集：基于上次采集时间戳 + URL 哈希去重
- 反爬策略：User-Agent 轮换、请求间隔（3-10s 随机）、代理 IP 池
- 数据标准化：不同网站的原始数据统一为内部格式
- 结构化输出：
```json
{
  "jobId": "唯一标识",
...
| 方向 | 交互方 | Topic | 数据流向 |
|------|--------|-------|----------|
| 上游 | `3.1 爬虫采集` | `raw-job-data` | 接收采集的原始岗位数据 |
| 下游 | `3.3 ETL清洗` | `raw-job-data` | 清洗模块消费此 Topic |
| 下游 | `3.4 离线分析` | `cleaned-job-data` | ETL 完成后写回的清洗结果 |

**具体功能：**

- Kafka 集群管理（3 Broker 最小部署）
- Topic 设计：
  - `raw-job-data`（原始数据，3 分区，保留 7 天）
  - `cleaned-job-data`（清洗后数据，3 分区，保留 30 天）
  - `collect-command`（采集指令下发：启动/停止/修改频率）
- Producer 端：异步发送 + 批量提交（batch.size=16KB），压缩类型 gzip
- Consumer 端：手动提交 Offset，保证至少一次消费语义
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 上游 | `3.2 消息队列` | 消费 Kafka Topic: `raw-job-data` | JSON 原始岗位 |
| 下游 | `3.2 消息队列` | 产入 Kafka Topic: `cleaned-job-data` | JSON 清洗后岗位 |
| 下游 | `4.3 HDFS` | 原始数据归档写入 | Parquet 文件 |
| 下游 | `4.4 Hive` | 清洗后数据写入 Hive 分区表 | Hive 表 |

**具体功能：**

- Spark Streaming / Spark Structured Streaming 消费 Kafka
- 清洗规则：
  - 空值处理：必填字段（岗位名、公司名）为空 → 丢弃
  - 薪资标准化：统一为月薪（K为单位），处理"面议""薪资open"等模糊表述
  - 城市标准化：二级城市映射到所属省份 + 城市层级（一线/新一线/二线/三线）
  - 学历标准化：映射为统一枚举（不限/大专/本科/硕士/博士）
  - 经验标准化：映射为统一枚举（不限/应届/1-3年/3-5年/5-10年/10年以上）
  - 技能提取：从岗位描述中基于预设技能词典（5000+ 技术关键词）做 NER 提取
  - 重复检测：jobId + sourceUrl MD5 双重去重
- 数据质量监控：每日统计各字段完整率、标准化成功率
...
| 方向 | 交互方 | 接口设计 | 数据格式 |
|------|--------|----------|----------|
| 上游 | `4.4 Hive` `4.3 HDFS` | 读取 Hive 表 / HDFS 文件 | |
| 下游 | 业务层 `2.2 数据分析服务` | 将分析结果写入 MySQL 统计结果表 | JDBC |
| 下游 | 存储层 `4.5 HBase` | 写入实时查询索引 | HBase Put |

**具体功能：**

- Spark SQL 离线批处理（每日凌晨 2:00 执行，由 DolphinScheduler / Crontab 调度）
- 计算任务：
  - 岗位维度：按类型/城市/行业分组统计数量、环比增长率
  - 薪资维度：avg / median / p25 / p75 / 区间分布
  - TopN：热门岗位 TOP20、高薪岗位 TOP20、热门技能 TOP30、热门城市 TOP10
  - 交叉分析：学历×薪资、城市×薪资、技能×薪资
- Hive 作为数据仓库层，存储清洗后的全量数据（按 dt 日期分区）
- MapReduce 用于历史批量数据的一次性大规模重算（如重新处理全年数据）
- 分析结果回写 MySQL 统计表（供业务层直接查询），HBase 存储明细索引（供实时/近实时查询）
...
| `dws_job_stat_daily` | `dt` | DWS 层：日汇总统计 |
| `dws_job_stat_monthly` | `month` | DWS 层：月汇总统计 |
| `ads_job_hot_skills` | `month` | ADS 层：热门技能分析 |
| `ads_city_salary_rank` | `month` | ADS 层：城市薪资排名 |

**交互设计：** Hive 元数据存储在 MySQL（Hive Metastore）；Spark SQL 通过 HiveContext 查询；ETL 模块通过 Spark 批量写入 Hive 分区表。
...
| 存储内容 | RowKey 设计 | 说明 |
|---------|------------|------|
| 岗位明细索引 | `{city}_{positionType}_{jobId}` | 按城市+类型快速检索 |
| 企业岗位历史 | `{companyId}_{timestamp}` | 企业招聘时间线 |
| 技能时间序列 | `{skillName}_{month}` | 技能需求月度变化 |

**交互设计：** 按需部署（项目初期可降级为 MySQL + Redis 组合）；Spark 批量写入 HBase；后端业务服务通过 HBase Java Client 查询。
...
| 组件 | 部署方式 | 说明 |
|------|---------|------|
| MySQL + Redis | Docker Compose 单机部署 | 数据持久化挂载 Volume |
| Spring Boot 服务 | Docker 容器化 | 每个服务独立镜像 |
| Vue 前端 | Nginx 静态资源 + Docker | 多阶段构建 |
| Kafka 集群 | Docker Compose（3节点） | KRaft 模式（无需 ZooKeeper）|
| Hadoop 生态 | Docker Compose | HDFS + Hive + Spark + HBase |
| 爬虫引擎 | Docker 容器 | Python + Scrapy 环境 |
...
- **Zookeeper**：Hadoop HA（NameNode 选主）、HBase Master 选主
- **Kafka KRaft**：Kafka 3.x 内置 Raft 共识（不再依赖 Zookeeper）
- **Spring Boot 服务**：Docker Compose 网络 + Nginx upstream 实现服务发现（项目规模较小，无需引入 Nacos/Consul 等注册中心）
...
4. **渐进式部署**：大数据组件（Hadoop/Spark/Hive/HBase）通过 Docker 容器化部署，降低环境搭建的复杂度；核心业务功能（用户、岗位查询、推荐）可独立于大数据组件运行，确保开发阶段的快速迭代。
